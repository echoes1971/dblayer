        ...
        if( xrv.hasChildNodes() ) {
          ...
          if( (xrv_type.tagName() == "i4") ||
            ...
            if( xrv_type.text() == "0" ) {
              ...
              this->QVariant::operator=(false);
            } else if(xrv_type.text() == "1") {
              //The second arg is a dummy that Qt needs for compatibility
              this->QVariant::operator=(true);
            }
            ...
            //printf("XRVariant::fromDomElement: text=%s\n", xrv_type.text().toStdString().c_str());
            //printf("XRVariant::fromDomElement: decode(text)=%s\n", XRBase64::decode( xrv_type.text() ).data());
            // RRA: inizio.
            this->QVariant::operator=(
                  QVariant( QByteArray::fromBase64( xrv_type.text().toStdString().c_str() ) )
            );
            //this->QVariant::operator=( QVariant( XRBase64::decode( xrv_type.text() ) ) );
            // RRA: fine.
...
bool XRVariant::isXmlRpcType(const QVariant& qv) {
      ...
      (qv.type() == QVariant::ByteArray) )
  {
	...
	QList<QVariant> qvList = qv.toList();
	for( it = qvList.begin(); it != qvList.end(); it++) {
      all_good = all_good && isXmlRpcType(*it);
    }
    return all_good;
  } else if( qv.type() == QVariant::Map ) {
    bool all_good = true;
    foreach (QVariant v, qv.toMap()) {
